...
class RecoveryRing(BaseVisualization):
    """Recovery score ring — color shifts based on recovery level."""
    ...
    def set_value(self, value: float, max_value: float = 100.0, label: str = "") -> None:
        self._max_value = max(max_value, 1.0)
        target = min(value, self._max_value)
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        painter.setFont(self._make_font(22, True))
        painter.setPen(QColor(colors.text_primary))
        painter.drawText(QRectF(0, cy - 24, self._ring_size + 20, 28), Qt.AlignCenter, f"{pct}")

        painter.setFont(self._make_font(10))
        painter.setPen(QColor(colors.text_disabled))
        painter.drawText(QRectF(0, cy + 4, self._ring_size + 20, 16), Qt.AlignCenter, "%")
